...
## Session Log
- Session 11: Route audit + Playwright test harness. Installed @playwright/test + Chromium. Created playwright.config.ts and "test" npm script. Fixed smoke.spec.ts and click-crawl.spec.ts — both had 5 broken/nonexistent routes: /inventory → /dashboard/inventory, /inventory/upload → /dashboard/inventory/new, /listings (no root page) → removed, /buy-orders → /dashboard/buy-orders, /profile (doesn't exist) → removed. Smoke spec now covers 18 real routes; click-crawl spec covers /login and /signup (only static unprotected routes safe for blind crawling). Found and fixed one real bug: /auth/callback called butterbase.auth.getSession() which does not exist in the SDK (the method is on the internal session manager, not auth). Fixed to use butterbase.auth.handleOAuthCallback() + onAuthStateChange listener instead. Also fixed click-crawl false positive: Next.js RSC prefetch requests (_rsc=) fire ERR_ABORTED on navigation — added filter to ignore these. Final result: 18/18 smoke tests pass, 2/2 click-crawl tests pass.
